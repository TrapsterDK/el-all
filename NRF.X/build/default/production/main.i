...
# 118 "./nrf24l01.h"
    ...
    {
        RD3 = 0; uint8_t status = SPI_write(command); for (uint8_t i = 0; i < len; i++) { data[i] = SPI_write(data[i]); } RD3 = 1; return status;
    }


    uint8_t nrf_command_with_data(uint8_t command, uint8_t *data, uint8_t len)
    {
        RD3 = 0; uint8_t status = SPI_write(command); for (uint8_t i = 0; i < len; i++) { SPI_write(data[i]); } RD3 = 1; return status;
    }
