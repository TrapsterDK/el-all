...
void UART_SetBaudRate(uint32_t v_baudRate_u32)
{
    ...
    if((v_baudRate_u32 >= 2400) && (v_baudRate_u32<=115200UL))
    {


        RegValue = (uint8_t)(uint32_t)((20000000UL / (64*(uint32_t)v_baudRate_u32)) - 1);
    }
    else
    {

        RegValue = (uint8_t)(uint32_t)((20000000UL / (64*(uint32_t)9600)) - 1);
    }
...
void UART_TxNumber(uint8_t v_numericSystem_u8, uint32_t v_number_u32, uint8_t v_numOfDigitsToTransmit_u8)
{
    ...
    else
    {
        for(i=0;i<v_numOfDigitsToTransmit_u8;i++)
        {
            ...
            else if( (v_numOfDigitsToTransmit_u8 == 0xffu) ||
                    (v_numOfDigitsToTransmit_u8 > 10u))
            {
    ...
    for(ptr = argList; *ptr != '\0'; ptr++)
    {
        ...
        if(ch == '%')
        {
           ...
           if((ch>=0x30) && (ch<=0x39))
            ...
            case 'C':
            case 'c':
                ch = (*(uint8_t *)__va_arg((*(uint8_t **)argp), (uint8_t)0));
                UART_TxChar(ch);
                break;

            case 'd':
                v_num_s16 = (*(sint16_t *)__va_arg((*(sint16_t **)argp), (sint16_t)0));
                if(v_num_s16<0)
                 {
                   v_num_s16 = -v_num_s16;
                   UART_TxChar('-');
            ...
            case 'u':
                v_num_u16 = (*(uint16_t *)__va_arg((*(uint16_t **)argp), (uint16_t)0));
            ...
            case 'b':
                ...
                if(v_numOfDigitsToTransmit_u8 == 0xffu)
