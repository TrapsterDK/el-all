...
# 118 "./nrf24l01.h"
    ...
    {
        RC3 = 0; uint8_t status = SPI_write(command); for (uint8_t i = 0; i < len; i++) { data[i] = SPI_write(data[i]); } RC3 = 1; return status;
    }


    uint8_t nrf_command_with_data(uint8_t command, uint8_t *data, uint8_t len)
    {
        RC3 = 0; uint8_t status = SPI_write(command); for (uint8_t i = 0; i < len; i++) { SPI_write(data[i]); } RC3 = 1; return status;
    }
    ...
    while (1)
    {
        for (uint8_t ypos = 0; ypos < 4; ypos++)
        {
            ...
            if (x1 || x2 || x3)
            {
                ...
                while (1)
                {
                    uint8_t tpos = RA1 * 1 + RA0 * 2 + RA3 * 4;
                    if (tpos != checkpos &&
...
{
    uint8_t digit = 0;
    uint8_t old_password[4];
    arrcpy(old_password, password, 4);
    ...
    while (1)
    {
        ...
        case 11:
            ...
            break;
        case 10:
            arrcpy(password, old_password, 4);
            wrong();
            return;
        default:
            password[digit++] = key;
...
uint8_t check_password()
{
    ...
    while (1)
    {
        ...
        default:
            ...
            if (digit >= 4)
            {
                if (arrequal(current_password, password, 4))
                {
                    success();
                    return 1;
                }
